<template>
  <div class="relative form-content y-space-60 rxsection-b-space">
    <div class="container">
      <div class="row">
        <div
          class="flex-grow w-full px-4 flex-base-0"
          v-for="item in data.TitleContentMediaList"
          :key="item.id"
        >
          <div
            class="circle_list primary_list list_sm noBs"
            v-if="item.LongText"
          ...
          <div class="flex flex-wrap items-center mt-4">
            <div
              ...
              v-for="items in item.ImageCollection"
            >
              <span>
                <img
                  v-rxlazy="items"
              ...
              <br />
              <span class="w-12 text-center" v-if="items.id != 4811">Vs</span>
            </div>
          </div>
        </div>
        <div class="flex-initial w-full px-4 smd:w-auto">
            ...
            <div id="crmWebToEntityForm" class="white_guide_form-main">
              ...
              >
                ...
                <div class="white_guide_form">
                  <h2
                    class="mb-4 text-center h3 text-themedark-0 dark:text-themedark-0"
                    v-for="item in data.TitleContentMediaList"
                    :key="item.id"
                  >
                    {{ item.Title }}
                  ...
                  <div
                    class="flex items-center form-group form-group-checkbox custom-checkbox"
                  >
                    <input
                      type="checkbox"
                      name="LEADCF109"
...
export default {
  ...
  data() {
    return {
      ...
      te: "true",
      LEADCF122: "Flutter Approach Whitepaper",
      LEADCF123:
        "https://d2ms8rpfqc4h24.cloudfront.net/Flutter_Approach_Whitepaper_f4e4518916.pdf",
      LEADCF2: "White Paper",
      LEADCF4: "",
      LEADCF7: "",
      LEADCF8: "",
  ...
  methods: {
    getUrl() {
      try {
        // const route = this.$route.params.pathMatch; // ['guides', 'software-developoment-amid-covid-19']
        var urlPath = window.location.pathname;
        var pairs = urlPath[0] === "/" ? urlPath.substr(1) : urlPath;
        var route = pairs.split("/");
        const data = [
          {
            url: "why-flutter-app-development",
            thankyou: "whitepapers-thankyou",
          },
          {
            url: "software-developoment-amid-covid-19",
            thankyou: "guides-thankyou?guide=Remote-Software-Development",
          },
          {
            url: "remote-software-development",
            thankyou:
              "guides-thankyou?guide=Software-Development-AMID-COVID-19",
          },
        ];
        ...
        const thankyouPage = url.thankyou;
        return thankyouPage;
      } catch (error) {
      ...
      try {
        ...
        const data = [
          ...
          },
          {
            url: "software-developoment-amid-covid-19",
            props: {
              guide: "Remote-Software-Development",
            },
          },
          {
            url: "remote-software-development",
            props: {
              guide: "Software-Development-AMID-COVID-19",
            },
          },
        ];
        const url = data.find((i) => i.url === route[1]);
        ...
        const props = url.props;
        return props;
      } catch (error) {
        console.log("getting props error ", error);
      }
    },
    async recaptcha() {
      try {
      ...
      try {
        ...
        if (status == "200") {
          if (response.data.search("history.back()") !== -1) {
            ...
            const queryParams = this.getProps();
