<template>
  <div>
    ...
    >
      <div class="container" :id="item.GuideURL">
        <div class="items-center row">
          <div class="w-full px-4 sxl:w-7/12 smd:w-1/2">
            <div class="head-content">
              <h1>{{ item.Title }}</h1>
              <div
                class="contentWhite list_md banner-dashed"
                v-if="item.LongText"
          ...
          >
            <div class="form-block smd:bg-opacitybg-65 dark:bg-themedark-65">
              <h3 class="text-center smd:text-themedark-10 text-themelight-0">Get Free Consultation</h3>
                ...
                <div id="crmWebToEntityForm" class="radix-zoho">
                  <form @submit.prevent="recaptcha">
                    <div class="pt-3 form_row row justify-content-center">
                      <div class="w-full px-4 pb-5 sm:w-1/2">
                        <input
                          type="text"
                          maxlength="80"
                      ...
                      <div class="w-full px-4 mb-0">
                        ...
                        <div class="submitCntcWrap">
                          <div class="mb-0 mt-4 frmactns text-center">
                            <button
                              type="submit"
                              value="Submit"
                              class="btn primary"
      ...
      try {
        ...
        if (status == "200") {
          if (response.data.search("history.back()") !== -1) {
            ...
            self.$tezRouter.push({
              path: "/thankyou",
              query: { location: this.data.GuideURL },
            });
          }
        }
      } catch (error) {
...
@layer utilities {
  .csd-banner {
    ...
    .banner-dashed {
      li {
        @apply md:mt-2 lg:mt-4 relative text-left;
        &:before {
          content: "";
          @apply bg-white inline-block align-middle;
          margin-right: 12px;
          width: 10px;
          height: 2px;
        }
